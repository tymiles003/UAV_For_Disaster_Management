    ...
    def estimate_paf(peaks, heat_mat, paf_mat):
        ...
        for human_id in range(pafprocess.get_num_humans()):
            ...
            if is_added:
                ...
                #print("\n\n")
                #print(humans)
                #print("humans should have been printed \n\n")
                #print("\n\n")
        return humans
...
class TfPoseEstimator:
    ...
    def __init__(self, graph_path, target_size=(320, 240), tf_config=None):
        ...
        #print("\n\n Inside the __init__ for TfPoseEstimator \n\n")
        # load graph
        ...
        #print("\n\n Printing the tensor_output.shape now \n\n")
        #print(self.tensor_output.shape)
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        centers = {}
        #print("\n\n Inside the draw_humans funtion now \n")
        for human in humans:
    ...
    def angle(a,b,c):
        ...
        '''
        result = np.arctan2(a[1] - b[1] , a[0]-b[0]) - np.arctan2(c[1]-b[1],c[0]-b[0])
        angle = np.degrees(abs(result))
        return angle
    ...
    def get_angles(self,humans,centers):
        ...
        for human in humans:
            ...
            angles_of_body_per_human = {}
            human_number +=1
            for i,triplet in enumerate(common.angle_triplets):
                if triplet[0] not in human.body_parts.keys() or triplet[1] not in human.body_parts.keys() or triplet[2] not in human.body_parts.keys():
                    continue

                #print(centers[triplet[0]] , centers[triplet[1]] , centers[triplet[2]])
                a,b,c = centers[triplet[0]] , centers[triplet[1]] , centers[triplet[2]]
                #angles_of_body_per_human.append(TfPoseEstimator.angle(a,b,c))
                angles_of_body_per_human[i] = TfPoseEstimator.angle(a,b,c)
            
            angles_of_body[human_number] = angles_of_body_per_human
        #print("\n\n")
        #print(angles_of_body)

        return angles_of_body


    def get_final_angle_vector(self,angles_of_body):

        angle_vectors = {}
        for j in range(1,len(angles_of_body)+1):
            angle_vectors[j] = np.zeros((12,1))
            for i in common.default_angles:
                if(i not in angles_of_body[j].keys()):
                    angle_vectors[j][i] = common.default_angles[i]
                else:
                    angle_vectors[j][i] = angles_of_body[j][i]
